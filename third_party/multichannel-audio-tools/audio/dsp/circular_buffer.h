...
// respectively. It is assumed that the same data is not intended to be read
...
class CircularBuffer {
 public:
  ...
  std::pair<MutableSpan, MutableSpan> Write(int num_to_write) {
    CHECK_GT(capacity_, 0) << "Did you forget to call Init()?";
    DCHECK_LE(num_to_write, capacity());
    if (num_to_write == 0) {
    ...
    if (num_available_data_ + num_to_write > capacity_) {
      ...
      // Update the read pointer such that it points at the oldest sample.
  ...
  // This function should be seen as an alternative to Read(int, vector<T>*) for
  ...
  std::pair<Span, Span> Peek(int num_to_read) const {
    ...
    DCHECK_GT(num_to_read, 0);
    DCHECK_LE(num_to_read, NumReadableEntries());
  ...
  void Advance(int advance_by) {
    CHECK_LE(advance_by, num_available_data_);
    is_full_ = false;
