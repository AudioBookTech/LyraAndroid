...
void BackgroundLevelDetector::Init(const BackgroundLevelDetectorParams& params,
  ...
  CHECK_LT(params_.fast_smoother_cutoff_hz,
           params_.envelope_sample_rate_hz / 2);
  CHECK_LT(params_.slow_smoother_cutoff_hz,
           params_.envelope_sample_rate_hz / 2);
  CHECK_GT(params_.fast_smoother_cutoff_hz, 0);
  CHECK_GT(params_.slow_smoother_cutoff_hz, 0);
  CHECK_GT(params_.transient_sensitivity, 0);
  CHECK_LE(params_.transient_sensitivity, 1);
...
bool BackgroundLevelDetector::ProcessBlockInternal(
    Eigen::Map<const Eigen::ArrayXXf> input) {
...
void BackgroundLevelDetector::SetSlowSmootherCutoffHz(float cutoff_hz) {
  CHECK_LT(cutoff_hz, params_.envelope_sample_rate_hz / 2);
...
void BackgroundLevelDetector::SetBackgroundSmootherTimeConstant(
  ...
  CHECK_GT(time_constant_seconds,
