...
void SetGainsFromArray(const ArrayXf& gains_db,
                       ParametricEqualizerParams* eq_params) {
  CHECK_EQ(gains_db.size(), eq_params->GetTotalNumStages() + 1);
...
void SetFrequenciesFromArray(const ArrayXf& log_frequencies_hz,
                             ParametricEqualizerParams* eq_params) {
  CHECK_EQ(log_frequencies_hz.size(),
...
void SetFrequenciesAndQualityFactorsFromArray(
  ...
  CHECK_EQ(frequencies_and_quality_factors.size(), 2 * half_size);
...
bool ParametricEqualizerParams::IsStageEnabled(int index) const {
  DCHECK_LT(index, GetTotalNumStages());
...
int ParametricEqualizerParams::GetNumEnabledStages() const {
  int num_stages = 0;
  for (uint8_t enabled : stages_enabled_) {
...
float FitParametricEqualizer(absl::Span<const float> frequencies_hz,
  ...
  CHECK_EQ(log_frequencies_and_quality_factors.size(), 2 * num_stages);
  searcher.SetSimplexFromStartingGuess(log_frequencies_and_quality_factors,
                                       0.1);

  // Set constraints.
  CHECK_GT(fit_params.min_frequency_hz, 0);
  CHECK_GT(fit_params.max_frequency_hz, 0);
  CHECK_LT(fit_params.min_frequency_hz, fit_params.max_frequency_hz);
